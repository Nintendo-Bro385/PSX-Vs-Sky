...
{	
    ...
    menu.msky = Char_Msky_New(FIXED_DEC(-70,1), FIXED_DEC(20,1));
	...
	//Fallthrough
        ...
        case MenuPage_Freeplay:
		{
			static const struct
			{
				...
				{StageId_1_1, 0xFF9271FD, "WIFE FOREVER", 2, 0,},
				{StageId_1_2, 0xFF9271FD, "SKY", 2, 1,},
				{StageId_1_3, 0xFF9271FD, "MANIFEST", 3, 2,},
			};
			...
			switch (menu_options[menu.select].music)
			{
				...
				{	
					if (pad_state.press & PAD_R1)
					{
						//Play Get Rich Music
						Audio_StopXA();
						Audio_PlayXA_Track(XA_Wifeforever, 0x40, 1, 1);
						Audio_WaitPlayXA();
					}
					if (pad_state.press & PAD_L1)
					{
				...
				{	
					if (pad_state.press & PAD_R1)
					{
						//Play Short Sale Music
						Audio_StopXA();
						Audio_PlayXA_Track(XA_Sky, 0x40, 3, 1);
						Audio_WaitPlayXA();
					}
					if (pad_state.press & PAD_L1)
					{
				...
				{	
					if (pad_state.press & PAD_R1)
					{
						//Play Bankrupt Music
						Audio_StopXA();
						Audio_PlayXA_Track(XA_Manifest, 0x40, 1, 1);
						Audio_WaitPlayXA();
					}
					if (pad_state.press & PAD_L1)
					{
