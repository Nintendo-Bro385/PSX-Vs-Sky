...
#include "character/sky.h"
#include "character/skya.h"
#include "character/skym.h"
#include "character/skycut.h"
...
static void Stage_LoadState(void)
{
	...
	if (stage.stage_id == StageId_1_4){pausestage = "CLUCKED";}
    ...
    switch (stage.state)
    {
        case StageState_Play:
        {
        	...
        	if((stage.stage_id == StageId_1_5 || stage.stage_id == StageId_1_6))
        		...
        		nohud=0;
        	if(stage.paused == false && stage.stage_id != StageId_1_5 && stage.stage_id != StageId_1_6)
        	{
			if (pad_state.press & PAD_START)
			{
	    ...
	    if (stage.prefs.botplay && stage.stage_id != StageId_1_5 && stage.stage_id != StageId_1_6)
	    {
                        ...
                        if(stage.stage_id == StageId_1_6)
			{
				...
				{
					Audio_StopXA();
					Audio_PlayXA_Track(XA_Bend, 0x40, 2, 0);
					Audio_WaitPlayXA();
				}
				else if(stage.ending==2)
				{
					Audio_StopXA();
					Audio_PlayXA_Track(XA_Fest, 0x40, 1, 0);
					Audio_WaitPlayXA();
				}
				else
				{
					Audio_StopXA();
					Audio_PlayXA_Track(XA_Gend, 0x40, 1, 0);
					Audio_WaitPlayXA();
				}
			}
			else
			{
                ...
                else
                {
					...
					if (stage.stage_id == StageId_1_3)
							    	{
							    		int randomending = RandomRange(1, 30);
							    		if(randomending!=13)
							    		{
								    		if(stage.misses <= 14)
								    		{
    ...
    //Fallthrough
        ...
        case StageState_Dialogue:
        {
			...
			switch (stage.stage_id)
			{
				...
				case StageId_1_5:
					dialoguep = sky2dia;
					dialogue_final = COUNT_OF(sky2dia);
				break;
				case StageId_1_3:
